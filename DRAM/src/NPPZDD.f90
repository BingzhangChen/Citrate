...
real :: pp_PN  = 0d0
real :: CHLtot = 0d0
real :: muNet,QN,QP,alphaG,Kp,gmax,RDN,RDN2,mz,PHYtot1
...
!! Phytoplankton section:
   ...
   ! MONOD(Temp, PAR,NO3,PO4,mu0,Qnmin,Qpmin,aI0,bI0,KN,KP,DFe,KFe,muNet,QN,QP,theta,SI,Lno3)
     CALL MONOD(Temp(k), PAR(k), NO3,1d0,mu0(i),params(iQ0N),Qpmin, &
                 aI0(i),bI0(i),          &
                 KN(i),KPO4, DFe(k), KFe_, muNet, QN, QP,          &
                 Varout(oTheta(i),k),    &
                 Varout(oSI(i),k),       &
                 Varout(oLno3(i),k))
    ...
    ! Calculate phytoplankton growth rate, theta, and QN based on environmental conditions
     CALL MONOD(Temp(k), par_, NO3, 1d0,mu0(i),params(iQ0N),Qpmin, &
                 aI0(i),bI0(i),  &
                 KN(i),KPO4, DFe(k), KFe_, muNet, Varout(oQN(i),k), QP, &
                 Varout(oTheta(i),k),   &
                 Varout(oSI(i),k),                                      &
                 Varout(oLno3(i),k))

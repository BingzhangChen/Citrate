...
if (INCLUDESIZE) then
   allocate(Size_out(sum(nrow(5,:)),4), STAT = AllocateStatus)
   IF (AllocateStatus /= 0) STOP "Problem in allocating Size_out!"
endif
...
if (INCLUDESIZE) then
   call Timestep(d_C_Chl,TIN_out,CHL_out, NPP_out,PON_out,Size_out)
else
   call Timestep(d_C_Chl,TIN_out,CHL_out, NPP_out,PON_out)
endif
...
DO j = 1, Nstn
   Do i = 1, NDTYPE
     ...
     case(4)   ! PON
       ...
       MOD_data = PON_out((kk(i)+1):(kk(i)+nrow(4,j)),1)
     case(5,6,7,8)   ! Size-fractionated Chl
       OBS_data = SizeData((kk(i)+1):(kk(i)+nrow(5,j)),i-2)
       MOD_data = Size_out((kk(i)+1):(kk(i)+nrow(5,j)),i-4)
     case default
   ...
   Enddo
ENDDO
if (allocated(Size_out)) deallocate(Size_out)

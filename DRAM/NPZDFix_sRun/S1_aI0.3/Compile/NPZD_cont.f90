...
!! ZOOplankton section:
   ...
   do i=1,M
     ...
     if (x(i) .gt. -3 .and. x(i) .le. 15.) then 
      ...
      KFe_=params(iKFe)*exp(alphaFe*x(i))
       call MONOD(tC,par_,NO3,mu0,params(iQ0N),aI0,bI0,KN,Fe,KFe_, &
                  mu, QN, theta, SI, Lno3)
   ...
   !   Here the PMU_1 is already the normal PMU + log(10) (to avoid negative values)
       ...
       call MONOD(tC,PAR(k),NO3,mu0,params(iQ0N),aI0,bI0,KN,Fe,KFe_,&
                  muNet1, QN1, theta1, SI1, Lno31)
...
!All rates have been multiplied by dtdays to get the real rate correponding to the actual time step
   ...
   Varout(omuAvg,   k) = Varout(omuNet(1),k)
   ...
   Varout(odgdl    ,k) = dgdlbar
   Varout(od2gdl   ,k) = d2gdl2bar

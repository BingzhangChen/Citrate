...
if (Model_ID .ne. NPZDFix .and. Model_ID .ne. NPPZDD .and. Model_ID .ne. NPZD2sp .and. Model_ID .ne. NPZDcont .and. Model_ID .ne. NPZDdisc) then
...
case(Geidersimple,Geiderdisc, NPZDFix,NPPZDD, NPZD2sp,NPZDdisc,NPZDCONT, NPZDFixIRON, GeidsimIRON)
  ...
  if (Model_ID .eq. NPZDcont .or. Model_ID .eq. NPZDFix .or. Model_ID .eq. NPPZDD .or. Model_ID.eq.NPZD2sp .or. Model_ID.eq.NPZDdisc .or. Model_ID .eq. NPZDFixIRON) then
     MaxValue(iaI0_C) =0.1
     MinValue(iaI0_C) =7D-3
  ...
  if (Model_ID==NPZDdisc .or. Model_ID==NPZDCONT) then
     MaxValue(ialphamu)=0.5
     MinValue(ialphamu)=0.1
     MaxValue(ibetamu) =0.
     MinValue(ibetamu) =-0.1
...
!rt2pi  : square root of 2*pi
     ...
     if (.not. FavorNPP) then 
        lprob=lprob-DBLE(NDPTS(k,j))*log(rt2Pi*sig(oi))-ss(oi)/(2d0*sig(oi)**2)
     else
        lprob=lprob-log(rt2Pi*sig(oi))                                   &
                - (ss(oi)/(2d0*sig(oi)**2))/DBLE(NDPTS(k,j))
     endif
!!$       lprob = lprob - ss(k)/(2.0d0*sig(k)**2)
  enddo
Enddo

! To restore the effects of total number of observations (DRAM_0.7)
if (FavorNPP) lprob=lprob*DBLE(MAXVAL(NDPTS))

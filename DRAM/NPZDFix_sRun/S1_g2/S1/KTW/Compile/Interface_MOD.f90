...
case(Geidersimple,Geiderdisc, NPZDFix, NPZDdisc, NPZDFixIRON, GeidsimIRON)
  ...
  if (Model_ID .eq. NPZDFix .or. Model_ID .eq. NPZDdisc .or. Model_ID .eq. NPZDFixIRON) then
     MaxValue(itheta) =  0.5
     MinValue(itheta) =  0.02
  endif
  ...
  if (Model_ID==NPZDdisc .or. Model_ID==Geiderdisc) then
     MaxValue(ialphamu)=0.3
     ...
     if(kill_the_winner) then
        MaxValue(ialphaG)=2.0
        MinValue(ialphaG)=1.0
     endif
  endif
!-------------------------------
case(EFTsimple, EFTdisc, EFTcont, EFTsimIRON)
...
do k = 1, NPar
     ...
     Ipv(k)   = Apv(k)
     if (.not. singlerun) Params(k)= Apv(k)
enddo
...
!rt2pi  : square root of 2*pi
     ...
     lprob = lprob - NDPTS(k,j)*log(rt2Pi*sig(oi)) - ss(oi)/(2d0*sig(oi)**2)

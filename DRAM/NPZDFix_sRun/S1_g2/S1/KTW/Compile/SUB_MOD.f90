...
real              ::  TIN_out(sum(nrow(1,:)),1)
real              ::  CHL_out(sum(nrow(2,:)),1)
real              ::  NPP_out(sum(nrow(3,:)),1)
real              :: d_C_Chl
...
if (Model_ID == EFTdisc .or. Model_ID == EFTcont .or. Model_ID==NPZDdisc) then
   allocate(Size_out(sum(nrow(4,:)),4), STAT = AllocateStatus)
...
params(:) = pars(:)
if (Model_ID == EFTdisc .or. Model_ID == EFTcont .or. Model_ID==NPZDdisc) then
   ! Here Timestep only give the output of one station
   call Timestep(d_C_Chl,TIN_out,CHL_out, NPP_out,Size_out)
else
   call Timestep(d_C_Chl,TIN_out,CHL_out, NPP_out)
endif
...
DO j = 1, Nstn
   Do i = 1, NDTYPE
     ...
     case(1)   ! Nitrate
       ...
       MOD_data = TIN_out((kk(i)+1):(kk(i)+nrow(1,j)),1)
     case(2)   ! CHL
       OBS_data = CHLData((kk(i)+1):(kk(i)+nrow(2,j)),3)
       MOD_data = CHL_out((kk(i)+1):(kk(i)+nrow(2,j)),1)
     case(3)   ! NPP
       OBS_data = NPPData((kk(i)+1):(kk(i)+nrow(3,j)),3)
       MOD_data = NPP_out((kk(i)+1):(kk(i)+nrow(3,j)),1)
     case(4,5,6,7)   ! Size-fractionated Chl
       OBS_data = SizeData((kk(i)+1):(kk(i)+nrow(4,j)),i-1)
       MOD_data = Size_out((kk(i)+1):(kk(i)+nrow(4,j)),i-3)
     case default
   ...
   !subroutine transform(N,x,xmin,xmax,convert,y)
     ...
     do don = (k+1),(k+NDPTS(i,j))
        ssqe(i+(j-1)*NDTYPE) = ssqe(i+(j-1)*NDTYPE) + (modval(don)-obsval(don))**2 
     enddo

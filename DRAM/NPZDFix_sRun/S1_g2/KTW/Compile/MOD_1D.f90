...
integer, private, parameter :: eDFe       = 6
integer, private, parameter :: TNFo       = eDFe  ! TOtal number of forcings

! Total of observation times in forcing data
! Use Taketo's Aks data for K2 and my own ROMS data for S1 
character(LEN=5), private, parameter :: LabelForc(TNFo) = (/'temp','NO3','Aks','wROMS','par', 'DFe'/)
integer,          private, parameter :: NFobs(TNFo)     = (/    12,   12, 360,     12,  12,   12 /)
...
real, private, target :: obs_time_DFe( NFobs(eDFe) , Nstn)
...
real, private :: obs_DFe( N_DFe ,  1+NFobs(eDFe ))
...
real, private, target :: VDFe( nlev, NFobs(eDFe) , Nstn)
...
Select case(Model_ID)
  case(NPZDFix, Geidersimple, EFTsimple, NPZDFixIRON, GeidsimIRON, EFTsimIRON)
     ! Data types must be the same for different stations.
     ! But can be different for different models
     NDTYPE = 3  !TIN, CHL, PP
  case(NPZDdisc, EFTdisc,EFTcont, Geiderdisc)
...
If (Model_ID .eq. EFTdisc .or. Model_ID .eq. EFTcont .or.            &
    Model_ID .eq. Geiderdisc  .or. Model_ID .eq. NPZDdisc) then
  DataLabel(itP10) = 'P10'
  DataLabel(itP03) = 'P03'
  DataLabel(itP01) = 'P01'
  DataLabel(itP_1) = 'P_1'
...
! Assign the data dimension, must be consistent with external file:
  if (trim(Stn(j)) .eq. 'S1') then
      ...
      if (Model_ID .eq. EFTdisc .or. Model_ID .eq. EFTcont .or.     &
          Model_ID .eq. NPZDdisc.or. Model_ID .eq. Geiderdisc) then
...
if (Model_ID == EFTdisc  .or. Model_ID == EFTcont .or.               &
    Model_ID == NPZDdisc .or. Model_ID == Geiderdisc) then
...
do j = 1, Nstn
   ...
   if (do_IRON) then  ! Read Iron data:
      call Readcsv(forcfiletime(eDFe), 1,NFobs(eDFe), obs_time_DFe(:,j)) 
      obs_time_DFe(:,j)  = obs_time_DFe(:,j) *3d1*dble(d_per_s)
   endif
   ...
   ! Read external DFe data:
   If (do_IRON) then
       call Readcsv(forcfile(eDFe), N_DFe, size(obs_DFe,2), obs_DFe) 

       ! Interpolate external DFe data:
       call gridinterpol(N_DFe,NFobs(eDFe),obs_DFe(:,1),         &
             obs_DFe(:, 2:(NFobs(eDFe)+1)),                      &
             nlev, Z_r, VDFe(:,:,j)) 
   Endif
...
DO jj = 1, Nstn
  ...
  if (savefile) then
    ...
    ! Improve the output routine: merge all data into one output file (Oct. 15 2016)
...
!%%%%%%%%%%%%%%%%%%%%%%%%For each time step, read in external environmental data%%%%%%%%%%%%%%%
    ...
    ! Interpolate DFe data throughout the water column:
     if (do_IRON) then
        pb => obs_time_DFe(:,jj)
        nn =  size(pb,1)
        pc => VDFe(:,:,jj)
        call time_interp(int(current_sec), nn, nlev,pb,pc, DFe)
    ...
    ! Calculate DATE OF the YEAR (DOY)
      ...
      ! Save DFe data into the output file:
         write(10, 2) Labelout(oDFe),  it, current_day, DFe
   ...
   If ((NDays-current_day) .le. 360) Then
     ...
     Do i=1, sum(nrow(:,jj))
        ...
        IF (DOY .eq. current_DOY) then
          ...
          if ((Model_ID.ne.NPZDFix) .and. (Model_ID.ne.NPZDdisc) .and. (Model_ID.ne.NPZDFixIRON) ) then

subroutine EFT_size(PMU, NO3, tC, par_, muNet, QN, Theta, LSI, Lno3)
use bio_MOD
implicit none
!INPUT PARAMETERS:
...
if (do_IRON) then
   K0Fe_= params(iKFe)
   KFe  = ScaleTrait(PMU, K0Fe_,alphaFe) !The half saturation constant of Fe at average size
endif
    
!Qs      = ScaleTrait(PMU, params(iQ0N), params(ialphaQ))/2d0
Qs      = params(iQ0N)/2d0
mu0hat  = dtdays*tf_p*params(imu0)* exp(alphamu*PMU + betamu*PMU*PMU)

! X: a scratch variable
X       = 0d0
if (do_IRON) then
  mu0hat= mu0hat *DFe_/(DFe_ + KFe)
  X     = alphaFe*KFe /(DFe_ + KFe) 
endif
...
aI = ScaleTrait(PMU, dtdays*params(iaI0), alphaI)
...
! Potential maximal nutrient-limited uptake
  ...
  Kn     = ScaleTrait(PMU, params(iKN), params(ialphaKN)) 

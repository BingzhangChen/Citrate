...
if (Model_ID==NPZDdisc) then
  ...
  NPHY = 1
  do i = 1, Nstn
     if (trim(Stn(i)) .eq. 'K2') DO_IRON = .TRUE.
  enddo
...
if (Model_ID == EFT2sp .OR. Model_ID==EFTPPDD .OR. Model_ID==NPZD2sp .OR. Model_ID==NPPZDD) then
    ...
    if (Model_ID==NPZD2sp .OR. Model_ID==NPPZDD) then
      ...
      ibI0B =  igmax  + 1   !The parameter for photoinhibition
       imz  =  ibI0B  + 1
...
elseif (nutrient_uptake.eq.2) then
   if (Model_ID==NPZD2sp .or. Model_ID==NPZDFix .or. Model_ID==NPPZDD .or. Model_ID==NPZDcont) then
...
if (Model_ID==NPZDFix.or.Model_ID==NPZD2sp.or.Model_ID==NPPZDD.or.Model_ID==NPZDdisc.or.Model_ID==NPZDFixIRON.or.Model_ID==NPZDcont) then
   iaI0_C  =  iwDET    + 1
...
params(imz)  =0.1
params(igmax)=1.0
if (Model_ID == NPZDdisc .or. Model_ID == NPZDFix .or. Model_ID==NPZDcont) then
   ! Update parameters on Nov. 9 2016
   params(imu0)     = 1.168695
elseif (Model_ID == EFTdisc .or. Model_ID==EFTcont .or. Model_ID==EFTsimple .or. Model_ID==EFTsimIRON .or. Model_ID==EFT2sp) then
   params(imu0)     = 2.5
else
   params(imu0)     = 2.5d0
endif
...
if (Model_ID .eq. NPZD2sp .OR. Model_ID .eq. EFTdisc .OR. Model_ID .eq. EFT2sp .OR.   &
 Model_ID.eq.NPZDCONT .OR. Model_ID.eq.NPPZDD .OR. Model_ID .eq. EFTPPDD) then
...
!
if(Model_ID==NPZDdisc.or.Model_ID==NPZD2sp.or.Model_ID==NPPZDD.or.Model_ID==NPZDFix .or. Model_ID==NPZDFixIRON .or. Model_ID==NPZDcont) then
...
if (Model_ID .ne. NPZDFix .and. Model_ID .ne. NPZDcont .and. Model_ID .ne. NPZDdisc .and. Model_ID .ne. NPZD2sp .and. Model_ID .ne. NPPZDD) then
ParamLabel(iaI0    ) = 'aI0'

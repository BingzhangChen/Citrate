...
! !LOCAL VARIABLES:
   integer            :: i,n
   REAL               :: rnd(npar),rnd_var_inv
   REAL,parameter     :: visc_back=0.e-6,rnd_var=0.333333333
   REAL               :: depth,dz(nlev),dzn(nlev),step,zp_old
...
!     local viscosity calculation
      if (visc_corr) then ! correction suggested by Visser [1997]
         ...
         zloc=zp(n)+0.5*(dzn(zi(n))+w)*dt  ! Negative

         do while (zloc .lt. -depth .or. zloc .gt. _ZERO_)  ! Particles out of the grid
            if (zloc .lt. -depth) then  ! Too deep
               zloc=-depth+(-depth-zloc) ! bounce back?
            else
               zloc=-zloc
            end if
         end do
         step = zloc-zp(n)   ! The distance traveled
